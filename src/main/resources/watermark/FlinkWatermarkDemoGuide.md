...
## 读前扫盲：Watermark 解决的是“事件时间走到哪了”

在流处理中，事件到达顺序经常和事件真实发生顺序不同。例如先收到 `ts=+10s`，再收到 `ts=+3s`，这叫乱序。如果窗口只看“当前收到的最后一条数据”，就无法判断一个时间段是不是已经收齐。

Watermark 可以先理解成 Flink 对事件时间进度的一句声明：

> 到目前为止，我认为不会再有早于某个时间点的数据，或者即使来了也要按迟到数据处理。

本 Demo 使用 `forBoundedOutOfOrderness(5s)`，意思是允许事件时间最多乱序 5 秒。它不改变事件的 `ts`，只是影响窗口什么时候认为“可以结算”。因此看日志时要分清两条时间线：

| 时间线 | 含义 | 本 Demo 中怎么看 |
|-------|------|-----------------|
| Event Time | 事件自带的业务时间，即 `WatermarkDemoEvent.ts` | 测试数据里的 `BASE + Ns` |
| Processing Time | Flink 机器处理这条消息的系统时间 | 发送测试数据时的真实等待和日志打印时间 |
| Watermark | Flink 估计的事件时间进度 | `[WM-MONITOR] currentWatermark` |

入门时先记住三个结论：

| 结论 | 为什么重要 |
|------|------------|
| Watermark 单调不减 | 迟来的旧事件不会把整体时间进度拉回去 |
| 多输入 / 多分区下游取最小 Watermark | 一个分区或一个 union 支路沉默，就可能拖住整个下游窗口 |
| `withIdleness` 用来处理空闲输入 | 某个输入长时间无数据时，可以把它临时标记为空闲，避免一直卡住最小 Watermark |

所以本文不是先讲“窗口怎么聚合”，而是先讲“窗口为什么迟迟不触发”。如果你看到 Job 有数据输入但没有 `[WINDOW-FIRED]`，第一反应应该是检查 Watermark 是否推进，而不是先怀疑聚合逻辑。
