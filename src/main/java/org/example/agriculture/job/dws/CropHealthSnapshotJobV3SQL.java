...
public class CropHealthSnapshotJobV3SQL {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 注册Kafka数据源表 - 植物健康详情
        tEnv.executeSql("CREATE TABLE plant_health_source (" +
        ...
        // 注册Kafka数据源表 - 土壤详情
        tEnv.executeSql("CREATE TABLE soil_source (" +
                "greenhouse_id STRING," +
                "sensor_id STRING," +
                "ts TIMESTAMP(3)," +
        ...
        // 注册Kafka数据源表 - 农事操作
        tEnv.executeSql("CREATE TABLE farm_operation_source (" +
                "greenhouse_id STRING," +
                "timestamp BIGINT," +
        ...
        tEnv.executeSql("CREATE TABLE clickhouse_sink (" +
                ...
                "PRIMARY KEY (greenhouse_id, stat_time) NOT ENFORCED" +
                ") WITH (" +
                "'connector' = 'jdbc'," +
                "'url' = 'jdbc:clickhouse://192.168.1.124:8123/default'," +
                "'table-name' = 'dws_greenhouse_crop_health_snapshot'," +
                "'username' = 'default'," +
                "'password' = '65e84be3'," +
        ...
        // 定义风险等级转换函数
        tEnv.executeSql("CREATE FUNCTION risk_rate AS (" +
                "risk_level STRING" +
                ") -> CASE " +
                "WHEN UPPER(risk_level) = 'LOW' THEN 0.1 " +
                "WHEN UPPER(risk_level) = 'MEDIUM' THEN 0.5 " +
                "WHEN UPPER(risk_level) = 'HIGH' THEN 0.9 " +
                "ELSE 0.0 " +
                "END");

        // 执行SQL查询，进行数据处理和JOIN
        String sql = "WITH plant_health_with_soil AS (" +
                "SELECT " +
                "ph.greenhouse_id, " +
                "ph.ts, " +
                "ph.crop_type, " +
                ...
                "s.soil_moisture, " +
                "s.soil_ec, " +
                "risk_rate(ph.disease_risk) AS disease_risk_rate, " +
                "risk_rate(ph.pest_risk) AS pest_risk_rate " +
                ...
                "greenhouse_id, " +
                "operation_type, " +
                "TIMESTAMP_LTZ(timestamp) AS operation_time, " +
                ...
                "LEFT JOIN latest_farm_operation lfo ON phs.greenhouse_id = lfo.greenhouse_id AND lfo.rn = 1;";

        tEnv.executeSql(sql);

        // 同时写入Kafka旁路输出
        String kafkaSql = "WITH plant_health_with_soil AS (" +
                "SELECT " +
                "ph.greenhouse_id, " +
                "ph.ts, " +
                "ph.crop_type, " +
        ...
        // 执行作业
        env.execute("DWS Greenhouse Crop Health Snapshot Processing (SQL)");
    }
}
