...
public class PlantHealthDetailJobV3 {

    public static void main(String[] args) throws Exception {
        // 设置执行环境
        final StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        env.enableCheckpointing(5000); // 每5秒一次checkpoint
        env.getConfig().setAutoWatermarkInterval(1000); // Watermark对齐
        ...
        // Step 1：定义Broadcast State
        MapStateDescriptor<String, SoilSensorData> soilBroadcastDesc = new MapStateDescriptor<>(
                "soil-broadcast",
                String.class,
                SoilSensorData.class
        );

        MapStateDescriptor<String, GreenhouseSensorData> envBroadcastDesc = new MapStateDescriptor<>(
                "env-broadcast",
                String.class,
                GreenhouseSensorData.class
        );

        // Step 2：soil → broadcast
        BroadcastStream<SoilSensorData> soilBroadcast = soilWithWm.broadcast(soilBroadcastDesc);

        // Step 3：env → broadcast
        BroadcastStream<GreenhouseSensorData> envBroadcast = envWithWm.broadcast(envBroadcastDesc);

        // Step 4：主流 connect broadcast（核心）
        DataStream<VisionWithSoil> visionWithSoil = visionWithWm
                .connect(soilBroadcast)
                .process(new BroadcastProcessFunction<PlantVisionData, SoilSensorData, VisionWithSoil>() {

                    @Override
                    public void processElement(PlantVisionData vision, ReadOnlyContext ctx, Collector<VisionWithSoil> out) throws Exception {
                        System.out.println("处理植物视觉数据: " + vision.getGreenhouseId());
                        ReadOnlyBroadcastState<String, SoilSensorData> state = ctx.getBroadcastState(soilBroadcastDesc);
                        SoilSensorData soil = state.get(vision.getGreenhouseId());
                        if (soil != null) {
                            System.out.println("找到土壤数据: " + soil.getGreenhouseId());
                            out.collect(new VisionWithSoil(vision, soil));
                        } else {
                            System.out.println("未找到土壤数据: " + vision.getGreenhouseId());
                        }
                    }

                    @Override
                    public void processBroadcastElement(SoilSensorData soil, Context ctx, Collector<VisionWithSoil> out) throws Exception {
                        System.out.println("广播土壤数据: " + soil.getGreenhouseId());
                        ctx.getBroadcastState(soilBroadcastDesc).put(soil.getGreenhouseId(), soil);
                    }
                });

        // Step 5：再次connect环境数据broadcast
        DataStream<DwdPlantHealthDetail> dwdStream = visionWithSoil
                .connect(envBroadcast)
                .process(new BroadcastProcessFunction<VisionWithSoil, GreenhouseSensorData, DwdPlantHealthDetail>() {

                    @Override
                    public void processElement(VisionWithSoil vs, ReadOnlyContext ctx, Collector<DwdPlantHealthDetail> out) throws Exception {
                        System.out.println("处理视觉+土壤数据: " + vs.getGreenhouseId());
                        ReadOnlyBroadcastState<String, GreenhouseSensorData> state = ctx.getBroadcastState(envBroadcastDesc);
                        GreenhouseSensorData env = state.get(vs.getGreenhouseId());
                        if (env == null) {
                            System.out.println("未找到环境数据: " + vs.getGreenhouseId());
                            return;
                        }

                        System.out.println("找到环境数据: " + env.getGreenhouseId());
                        ...
                        System.out.println("生成DWD数据: " + dwdData.getGreenhouseId());
                        out.collect(dwdData);
                    }

                    @Override
                    public void processBroadcastElement(GreenhouseSensorData env, Context ctx, Collector<DwdPlantHealthDetail> out) throws Exception {
                        System.out.println("广播环境数据: " + env.getGreenhouseId());
                        ctx.getBroadcastState(envBroadcastDesc).put(env.getGreenhouseId(), env);
                    }
                });

        // Step 6：Flink → Kafka（解耦）
        KafkaSink<String> kafkaSink = KafkaSink.<String>
                builder()
                .setBootstrapServers(kafkaProps.getProperty("bootstrap.servers"))
                .setRecordSerializer(KafkaRecordSerializationSchema.builder()
                        .setTopic("dwd_greenhouse_plant_health_detail")
        ...
        dwdStream
                .map(new MapFunction<DwdPlantHealthDetail, String>() {
                    ...
                    @Override
                    public String map(DwdPlantHealthDetail value) throws Exception {
        ...
        // 执行作业
        env.execute("DWD Greenhouse Plant Health Detail Processing V3");
    }
    ...
    private static DataStream<PlantVisionData> readPlantVisionData(StreamExecutionEnvironment env, Properties baseProps) {
        KafkaSource<String> kafkaSource = KafkaSource.<String>
                ...
                .setGroupId("plant-vision-consumer-v3")
        ...
        return env.fromSource(kafkaSource, WatermarkStrategy.noWatermarks(), "Plant Vision Kafka Source")
                .map(new MapFunction<String, PlantVisionData>() {
                    ...
                    public PlantVisionData map(String value) throws Exception {
                        System.out.println("接收到植物视觉数据: " + value);
                        try {
                            PlantVisionData data = objectMapper.readValue(value, PlantVisionData.class);
                            System.out.println("解析成功: " + data.getGreenhouseId());
                            return data;
                        } catch (Exception e) {
                ...
                .filter(new FilterFunction<PlantVisionData>() {
                    ...
                    public boolean filter(PlantVisionData data) throws Exception {
                        ...
                        if (result) {
                            System.out.println("过滤通过: " + data.getGreenhouseId());
                        } else {
                            System.out.println("过滤失败: " + (data == null ? "null" : data.getGreenhouseId()));
                        }
                        return result;
                    }
                });
    ...
    private static DataStream<SoilSensorData> readSoilSensorData(StreamExecutionEnvironment env, Properties baseProps) {
        KafkaSource<String> kafkaSource = KafkaSource.<String>
                ...
                .setGroupId("soil-sensor-consumer-v3")
                .setStartingOffsets(OffsetsInitializer.earliest())
                .setValueOnlyDeserializer(new SimpleStringSchema())
                .build();
    ...
    private static DataStream<GreenhouseSensorData> readEnvSensorData(StreamExecutionEnvironment env, Properties baseProps) {
        KafkaSource<String> kafkaSource = KafkaSource.<String>
                ...
                .setGroupId("env-sensor-consumer-v3")
                .setStartingOffsets(OffsetsInitializer.earliest())
                .setValueOnlyDeserializer(new SimpleStringSchema())
                .build();
     ...
     * Async I/O 维表扩展（预留）
     */
    public static class AsyncDimFunction extends org.apache.flink.streaming.api.functions.async.RichAsyncFunction<DwdPlantHealthDetail, DwdPlantHealthDetail> {
        ...
        @Override
        public void asyncInvoke(DwdPlantHealthDetail input, org.apache.flink.streaming.api.functions.async.ResultFuture<DwdPlantHealthDetail> resultFuture) {
            java.util.concurrent.CompletableFuture
                    .supplyAsync(() -> enrich(input), executor)
                    .thenAccept(result -> {
                        java.util.List<DwdPlantHealthDetail> results = new java.util.ArrayList<>();
                        results.add(result);
                        resultFuture.complete(results);
                    });
        }

        private DwdPlantHealthDetail enrich(DwdPlantHealthDetail input) {
            // 这里可以扩展：调用外部服务、查询Redis等
            return input;
        }
